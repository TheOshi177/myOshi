...
// Funkcja do pobrania szczegółów IP
...
// Funkcja do parsowania przeglądarki i systemu z User-Agent
function getBrowserAndOS($user_agent) {
    ...
    // ---- Przeglądarka ----
    if (strpos($user_agent, 'OPR') !== false) {
        $browser = 'Opera';
...
$stmt->execute([
    ...
    $details['city'] ?? '',
    $details['loc'] ?? '',
    $browser, // teraz: "Windows 11 – Chrome 119.0"
    $screen_total_width,
    $screen_total_height,
    $screen_width,
    $screen_height,
